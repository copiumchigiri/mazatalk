...
void main() {
  ...
  test('completeLesson pays coins once and is idempotent on replay', () async {
    ...
    await controller.addChild(const ChildProfile(id: 'c1', name: 'Test', age: 5));
  ...
  test('updateCheckpoint persists the resume position', () async {
    ...
    await controller.addChild(const ChildProfile(id: 'c1', name: 'Test', age: 5));
    await controller.updateCheckpoint('c1', lessonId: 'lesson_space', checkpoint: 2);
  ...
  test('purchaseSkin fails without enough coins and succeeds after earning them', () async {
    final (container, controller) = await makeController();
    addTearDown(container.dispose);
  ...
  test('daily streak: same day holds, next day increments, gap resets',
      () async {
    final (container, controller) = await makeController();
    addTearDown(container.dispose);
    await controller.addChild(const ChildProfile(id: 'c1', name: 'T', age: 5));

    Future<void> record(DateTime now) => controller.recordLessonResults('c1',
        xp: 10, answersTotal: 5, answersCorrectFirstTry: 5, now: now);
    ChildProfile child() =>
  ...
  test('recordLessonResults accumulates xp/accuracy and dedupes mistakes',
      () async {
    final (container, controller) = await makeController();
    addTearDown(container.dispose);
    await controller.addChild(const ChildProfile(id: 'c1', name: 'T', age: 5));

    await controller.recordLessonResults('c1',
        xp: 14, answersTotal: 5, answersCorrectFirstTry: 4,
        mistakes: ['u1l1:0']);
    await controller.recordLessonResults('c1',
        xp: 12, answersTotal: 6, answersCorrectFirstTry: 2,
  ...
  test('account switch swaps to the new account\'s children', () async {
    ...
    final containerA = ProviderContainer(overrides: [
      childRepositoryProvider.overrideWithValue(repoA),
    ]);

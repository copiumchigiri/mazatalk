...
void main() {
  ...
  /// default (no more Duolingo-style home screen) — these tests target the
  /// path/lesson screens directly, so jump there explicitly.
  ...
  testWidgets('full lesson: mistakes requeue, rewards paid, path advances', (
    ...
    await tester.pumpAndSettle();
    // Lesson one opens with the see / say / trace intro for «А».
    ...
    await tester.pump();
    expect(find.text('❌ Дахин оролдоно уу'), findsOneWidget);
    ...
    for (final activity in lesson.activities.skip(1)) {
      ...
      await tester.pump();
      expect(find.text('✅ Маш сайн!'), findsOneWidget);
    ...
    expect(find.text('Оновчтой байдал: 4/5'), findsOneWidget);
  ...
  testWidgets('a lesson with a match-pairs board plays through the player', (
    ...
    expect(find.text('${lesson.title} дууслаа!'), findsOneWidget);
    expect(find.text('Оновчтой байдал: 6/6'), findsOneWidget);
  });

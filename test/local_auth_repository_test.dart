...
void main() {
  ...
  group('LocalAuthRepository', () {
    test('signUp creates a persistent account and signs it in', () async {
      ...
      final result =
          await repo.signUp(email: 'Mom@Example.com', password: 'secret1');
      ...
      final login = await LocalAuthRepository()
          .login(email: 'mom@example.com', password: 'secret1');
      expect(login.isSuccess, isTrue);
    });

    test('signUp rejects invalid email, short password, and duplicates',
        () async {
      final repo = LocalAuthRepository();
      expect((await repo.signUp(email: 'notanemail', password: 'secret1')).error,
          isNotNull);
      expect(
          (await repo.signUp(email: 'a@b.com', password: '123')).error,
          isNotNull);

      expect((await repo.signUp(email: 'a@b.com', password: 'secret1')).isSuccess,
          isTrue);
      final dup = await repo.signUp(email: 'A@B.com', password: 'other12');
      expect(dup.error, contains('already exists'));
    });

    test('login fails identically for unknown email and wrong password',
        () async {
      final repo = LocalAuthRepository();
      await repo.signUp(email: 'a@b.com', password: 'secret1');
      final wrongPassword =
          await repo.login(email: 'a@b.com', password: 'nope123');
      final unknownEmail =
          await repo.login(email: 'ghost@b.com', password: 'secret1');
    ...
    test('resetPassword replaces the old password', () async {
      ...
      expect(
          await repo.resetPassword(
              email: 'ghost@b.com', newPassword: 'whatever'),
          isFalse);
      expect(
    ...
    test('provider sign-in creates a passwordless account and reuses it',
        () async {
      final repo = LocalAuthRepository();
      final first =
          await repo.signInWithProvider(accountId: 'phone-99119911');
      ...
      final passwordLogin =
          await repo.login(email: 'phone-99119911', password: 'anything');
  ...
  group('per-account child storage', () {
    ...
    test('signed-out repository reads empty and writes nowhere', () async {
      ...
      await signedOut
          .saveChildren([const ChildProfile(id: 'c1', name: 'X', age: 4)]);

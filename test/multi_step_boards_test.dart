...
Widget _host(Widget board) => MaterialApp(
  home: Scaffold(
    body: Padding(padding: const EdgeInsets.all(16), child: board),
  ),
);
...
void main() {
  testWidgets('MatchPairsBoard: solving cleanly reports clean=true', (
    ...
    final activity = matchPairs('Match', const [
      ('cat', '🐱'),
      ('dog', '🐶'),
      ('sun', '☀️'),
    ], Random(1));
  ...
  testWidgets(
    'MatchPairsBoard: a wrong match flashes and reports clean=false',
    (tester) async {
      final activity = matchPairs('Match', const [
        ('cat', '🐱'),
        ('dog', '🐶'),
      ...
      final wrongRight =
          activity.rightColumn[activity.correctOrder[0] == 0 ? 1 : 0];
